...
{
	Q_OBJECT
	Q_PROPERTY(QPointF firstBonePos READ firstBonePos WRITE setFirstBonePos NOTIFY firstBonePosChanged DESIGNABLE true USER true)
...
private:
	...
	const double step = 20.0;	//前进一步的步长
	
	QList<QPointF*> m_bonePoints;	//骨骼点坐标

	const double angleConstraint = M_PI_4/4;	//角度约束
	
	QList<qreal> m_constraintRadius;	//骨骼点约束半径

	//在骨骼点外绘制圆
	QList<qreal> m_bodyRadius{ 20, 25, 20, 15, 15, 
		15, 15, 15, 15, 15, 
		15, 15, 15, 15, 15, 
		15, 15, 15, 15, 15, 
		15, 15, 15, 15, 15, 
	...
	Direction direction();
	void moveStepDirection(Direction dirc);
